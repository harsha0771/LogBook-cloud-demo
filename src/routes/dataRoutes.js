...
router.get("/bulk/status/:processId", (req, res) => {
    ...
    if (!status) return res.status(404).send({ error: "Process not found" });
    res.json(status);
});
...
router.get("/read/:entity/:id", async (req, res) => {
    ...
    try {
        ...
        res.status(500).send({ error: "Error fetching item", details: error.message });
    }
});
...
router.put("/update/:entity/:id", async (req, res) => {
    ...
    try {
        ...
        const existingItem = result ? JSON.parse(result) : null;
        ...
        let updatedHashes = [];
        for (let [fieldKey, value] of Object.entries(updatedItem)) {
            const oldValue = existingItem[fieldKey];
            if (oldValue && oldValue != value) {
                updatedHashes.push(fieldKey);
                ...
                for (const element3 of valuies) {
                    ...
                    const hsData = await db.get("HashData:" + hashedText);
                    let hs = hsData ? JSON.parse(hsData) : null;
                    if (hs) {
                        hs[oldValue] = hs[oldValue] || {};
                        hs[oldValue][entity] = hs[oldValue][entity] || {};
                        hs[oldValue][entity][fieldKey] = hs[oldValue][entity][fieldKey] || [];
                        const index = hs[oldValue][entity][fieldKey].indexOf(id);
                        if (index > -1) hs[oldValue][entity][fieldKey].splice(index, 1);
        ...
        for (const fieldKey of updatedHashes) {
            await require("../utils/dbUtils").makeHash(updatedItem[fieldKey], fieldKey, entity, id);
        }
